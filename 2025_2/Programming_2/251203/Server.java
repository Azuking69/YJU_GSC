...
class ServerThread extends Thread {
	...
	public ServerThread(Socket server) {
		super(index + ""); // 현재 스레드 이름을 index로 부여
		index++;
		this.server = server;
	}
	...
	public void run() {
		...
		while (flag) {
			...
			try {
				...
				// 요기서 문제가 발생했다는건
				// 지금 스레드가 담당하고 있는 클라이언트가 종료되었다라고 추측할 수 있다.
				// 따라서 현재 스레드는 더 이상 구동시키지 말고 종료 해줄 필요가 있음!!
				System.out.print(this.getName() + "Thread에서 문제발생:");
				System.out.println("클라이언트에서 보내온 메시지 수신하다가 문제 발생됨");
				flag = false; // while 문이 더이상 동작하지 않게 됨 => 스레드 종료효과
			}
//			try {
			// 수신된 메시지를 모든 클라이언트로 송신
